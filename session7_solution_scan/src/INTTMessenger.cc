    ...
    fDirectory_(0)
{
  ...
  // note : for the INTT Box rotation and INTT L1 offset
  //////////////////////////////////////////////////////////////////////////////
  // setting the INTT L1 Y offset
  L1_offset_command_ = new G4UIcmdWithADouble( "/INTT/geom/setL1Offset", this );
  L1_offset_command_->SetGuidance( "Set the INTT L1 ladder Y-axis offset, unit mm, positive : goes down" );
  L1_offset_command_->SetDefaultValue( 0.0 );

  //////////////////////////////////////////////////////////////////////////////
  // setting the INTT Box rotation
  INTT_box_rotation_command_ = new G4UIcmdWithADouble( "/INTT/geom/setINTTBoxRotation", this );
  INTT_box_rotation_command_->SetGuidance( "Set INTT box rotation, unit degree, positive : goes down" );
  INTT_box_rotation_command_->SetDefaultValue( 0.0 );
...
void INTTMessenger::SetNewValue(G4UIcommand* command, G4String val)
{
  ...
  else if (command == L1_offset_command_)
  {
    // G4Tokenizer next( val );
    // L1_offset_ = StoD( next() );
    L1_offset_ = L1_offset_command_->GetNewDoubleValue( val );
  }
  else if (command == INTT_box_rotation_command_)
  {
    // G4Tokenizer next( val );
    // INTT_box_rotation_ = StoD( next() );
    INTT_box_rotation_ = INTT_box_rotation_command_->GetNewDoubleValue(val);
  }
}
...
void INTTMessenger::Print()
{
  ...
  G4cout << "  G4double  L1_offset_: " << L1_offset_ << endl;
  G4cout << "  G4double  INTT_box_rotation_: " << INTT_box_rotation_ << endl;
