  ...
  for( int i=0; i<trigger_IDs_.size(); i++ ) // loop over all hits colection of the silicon sensors
    {
      ...
      // G4cout<<" Test, EDEventAction, trigger_IDs_.size() : "<<trigger_IDs_.size()<<G4endl;

      auto hc = GetTriggerHitsCollection( trigger_IDs_[i], event );      
      //hc->PrintAllHits();
      
      // G4cout<<"EventAction test trigger  : "<<hc->entries()<<G4endl;
      ...
      for( int j=0; j<hc->entries(); j++ ) // loop over all hits in this hits collection
      {
        ...
        // G4cout<<"Test, scintillator energy : "<<energy<<" "<<i<<" "<<j<<G4endl;
        if( energy <= 0.0 ) // hits with finit energy deposit should pass this if statement
          continue;
        
        // G4cout<<"Test, scintillator energy pass cut : "<<energy<<" "<<i<<" "<<j<<G4endl;
        ...
        // G4cout<<"Test, scintillator energy pass cut : "<<energy<<" "<<i<<" "<<j<<" Pos : "<<x<<" "<<y<<" "<<z<<" eID : "<<event_id<<G4endl;
  ...
  for( int i=0; i<sensor_IDs_.size(); i++ ) // loop over all hits colection of the silicon sensors
    {
      ...
      // G4cout<<"EventAction test INTT  : "<<hc->entries()<<G4endl;
